"""
TypewriterPop Plugin-based ASS to HTML converter
プラグイン型TypewriterPopテンプレート実装
"""
...
class TypewriterPopTiming(NamedTuple):
    """タイミング情報"""
    text: str
    start_time_ms: int
    end_time_ms: int
...
class TypewriterPopPluginConverter(PluginConverterBase):
    """プラグイン型TypewriterPop ASS→HTML変換クラス"""
    
    def __init__(self):
        super().__init__()
        self.timings: List[TypewriterPopTiming] = []
    
    def get_template_config(self) -> TemplateConfig:
        """TypewriterPopテンプレートのプラグイン設定"""
        return TemplateConfig(
            template_name="typewriter_pop",
            navigation_unit="line",
            required_plugins=["auto_play", "typewriter_pop_display"],
            plugin_configs={
                "auto_play": {
                    "auto_start": True,
                    "initial_delay": 500
                },
                "typewriter_pop_display": {
    ...
    def parse_ass_file(self, ass_file_path: str) -> None:
        ...
        # メタデータ抽出
        self.metadata = ASSMetadataExtractor.extract_metadata(content, "TypewriterPop")
    ...
    def _get_template_title(self) -> str:
        """HTMLタイトルを返す"""
        return f"TypewriterPop Animation"
